...
Run the published image to provision the inventory and deploy LibreTranslate to a managed server (the mounted volume persists the inventory between the two runs):
...
docker run --rm -it \
  ...
  --include 'web-svc-libretranslate'
