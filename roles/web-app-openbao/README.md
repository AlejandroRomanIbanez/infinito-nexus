...
- **The seal key is pinned, and that is deliberate.** `meta/secrets.yml` marks it `rotatable: false`, so the per-deploy rotation skips it. The static seal carries one generation of slack, and rotating on every deploy spends all of it continuously: a run that skips a generation, or dies between the two AppRole pins, leaves the raft store readable only from a volume backup plus the key that wrapped it — and the recovery keys that would otherwise authorise a rekey were discarded at init. The key gains little from frequent rotation in exchange, because it lives in the vault-encrypted inventory either way, and an attacker holding that inventory holds the AppRole too and does not need the seal key to reach the data. Rotate it deliberately instead, by changing `secrets.credentials.seal_key` in the inventory: `templates/openbao.hcl.j2` then renders the `previous_key_id`/`previous_key` pair, the node unseals with the old key and rewraps to the new one, and the next deploy sees them equal again. The cost of pinning is that a leaked inventory backup stays valid against future volume snapshots instead of ageing out in two deploys.
- **The recovery key is kept, and it is the way back.** `bao operator init` issues it once, on the first deploy, and nothing ever reissues it — which is exactly what makes it useful: it is the only authority here that does not rotate. `tasks/01_init.yml` persists it through `sys-token-store`, the platform role for "tokens that already exist (e.g. created by bootstrap processes)". That puts it in `DIR_SECRETS`, which outlives both `MODE_RESET` (it removes `DIR_COMPOSITIONS` only) and a replaced swarm manager (`/opt/compose` is manager-local), each of which takes `.applied.json` with it. `svc-bkp-secrets-2-local` already backs that directory up. The inventory would be the more natural home, but no role writes `host_vars` — the inventory is written by the CLI, before the play. When neither the inventory AppRole nor the recorded one is accepted, `tasks/utils/recover.yml` mints a root token from the key via `bao operator generate-root` and hands over to `tasks/02_auth.yml`, which re-pins both halves to the inventory values. The deploy then continues instead of ending. Two costs are real: in swarm `DIR_SECRETS` is the NFS export, so the key is cluster-readable rather than manager-local, and a recovery key authorises `generate-root` and `rekey` — whoever reads that file owns the instance. It sits beside `tokens.yml` and `databases.csv`, which are the same class of secret.
- **A recovery key cannot decrypt the store, so it is not a backup of the seal key.** Initialising with an auto-seal yields *recovery* keys rather than unseal shares. They cannot decrypt the root key — only the seal key can — so they are not disaster-recovery material for the data; they authorise `operator generate-root` and `operator rekey`, nothing more. Losing the seal key still means the raft volume is unreadable, whatever else survives. What the recovery key buys is administrative access, which is a different failure and the one the role could not previously survive: a RoleID/SecretID changed out-of-band inside OpenBao used to leave no way back in at all.
...
- **Every deploy rotates the AppRole, and the role is built around that.** The platform regenerates generated-algorithm credentials on each deploy — that is how a changed Ansible password reaches the application instead of silently diverging from it. For OpenBao it means the `role_id`/`secret_id` a run is handed are *not* the ones the running instance was configured with, which would otherwise leave the AppRole answering `invalid role or secret ID`. The role therefore carries the transition itself:
  ...
  The record is written last, after both pins, so a run that dies partway leaves the previous generation intact and the next deploy still authenticates. The one uncovered window is a failure *between* the two pins — the RoleID re-pinned, the SecretID not, or the reverse — which leaves a pairing neither the inventory nor the state file describes. Two generations is the whole window for the AppRole, and discarding the state file while the volume survives puts the instance outside it: OpenBao still holds the previous SecretID, the inventory has already rotated past it, and the fallback has nothing left to read. `MODE_RESET` does exactly that — `sys-svc-compose/tasks/01_reset.yml` removes `DIR_COMPOSITIONS` wholesale while the named volumes persist. Neither case is fatal any more: the store stays readable because the seal key is pinned, and the recovery key in `DIR_SECRETS` survives the reset and rebuilds the AppRole. Purge the entity's volumes too when you reset — the same asymmetry breaks every entity that keeps a credential inside its volume, and those have no recovery key.
