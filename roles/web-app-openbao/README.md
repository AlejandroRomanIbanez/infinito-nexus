...
- **Every deploy rotates the AppRole, and the role is built around that.** The platform regenerates generated-algorithm credentials on each deploy — that is how a changed Ansible password reaches the application instead of silently diverging from it. For OpenBao it means the `role_id`/`secret_id` a run is handed are *not* the ones the running instance was configured with, which would otherwise leave the AppRole answering `invalid role or secret ID`. The role therefore carries the transition itself:
  ...
  The record is written last, after both pins, so a run that dies partway leaves the previous generation intact and the next deploy still authenticates. The one uncovered window is a failure *between* the two pins — the RoleID re-pinned, the SecretID not, or the reverse — which leaves a pairing neither the inventory nor the state file describes, and no recovery keys are kept to get back in. Two generations is the whole window for the AppRole. Discarding the state file while the volume survives puts the instance outside it: OpenBao still holds the previous SecretID, the inventory has already rotated past it, and the fallback has nothing left to read. `MODE_RESET` does exactly that — `sys-svc-compose/tasks/01_reset.yml` removes `DIR_COMPOSITIONS` wholesale while the named volumes persist. The store itself stays readable, because the seal key is pinned and still comes from the inventory, but no administrative path into it remains. Purge the entity's volumes too when you reset — the same asymmetry breaks every entity that keeps a credential inside its volume.
