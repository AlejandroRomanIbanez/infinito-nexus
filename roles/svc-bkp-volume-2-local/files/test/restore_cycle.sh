...
for volume in "${VOLUMES[@]}"; do
    ...
    rm -rf "${_wipe_mp:?}"/* "${_wipe_mp}"/.[!.]* "${_wipe_mp}"/..?* 2>/dev/null || true  # nocheck: shell-or-true -- grandfathered: worked in practice; TODO: sharpen to catch only the exact tolerated error
