...
Run the published image to provision the inventory and deploy FusionDirectory to a managed server (the mounted volume persists the inventory between the two runs):
...
docker run --rm -it \
  ...
  --include 'web-app-fusiondirectory'
