...
def _sh(args: list[str]) -> subprocess.CompletedProcess:
    return subprocess.run(args, cwd=COMPOSE_DIR, capture_output=True, text=True)


def _admin_creds() -> str:
    r = _sh(["compose", "exec", "-T", "stalwart", "printenv", "STALWART_RECOVERY_ADMIN"])
...
def _jmap(creds: str, calls: list, using: list[str] | None = None) -> dict:
    ...
    r = _sh([
        "compose", "exec", "-T", "stalwart", "curl", "-s", "-u", creds,
        "-H", "Content-Type:application/json", "--data-binary", "@/tmp/e2e.json",
...
def main() -> int:
    ...
    if "d" not in created:
        ...
        dom = next((d for d in got["methodResponses"][0][1]["list"] if d["name"] == DOMAIN), None)
        if not dom:
    ...
    resp = _jmap(admin, [["x:Account/set", {"create": {
        "a": {"@type": "User", "name": "alice", "domainId": domain_id,
              "credentials": {"0": {"@type": "Password", "secret": ALICE_PW}}},
        "b": {"@type": "User", "name": "bob", "domainId": domain_id,
              "credentials": {"0": {"@type": "Password", "secret": BOB_PW}}},
    }}, "c0"]])
    ...
    # 3. Send alice -> bob over SMTP submission (465, implicit TLS, authenticated).
    send = _sh([
        "compose", "exec", "-T", "stalwart", "curl", "-sS", "--url", "smtps://localhost:465",
        "--mail-from", f"alice@{DOMAIN}", "--mail-rcpt", f"bob@{DOMAIN}",
        "--upload-file", "/tmp/e2e_msg.txt", "--user", f"alice@{DOMAIN}:{ALICE_PW}", "-k",
    ])
    if send.returncode != 0:
        _cleanup(admin, domain_id, account_ids)
        sys.exit(f"FAIL: SMTP submission failed: {send.stderr[:300]}")

    # 4. Confirm receipt in bob's mailbox via JMAP (poll for delivery).
    found = False
    for _ in range(20):
        q = _jmap(f"bob@{DOMAIN}:{BOB_PW}", [
            ["Email/query", {"accountId": account_ids[1]}, "c0"],
            ["Email/get", {"accountId": account_ids[1],
                           "#ids": {"resultOf": "c0", "name": "Email/query", "path": "/ids"},
                           "properties": ["subject"]}, "c1"],
