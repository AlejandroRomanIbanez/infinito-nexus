...
Checks exactly what seed_mailu_maildir.py planted (also serving as the
...
Failures raise SystemExit instead of assert so the checks survive `python -O`.
"""
...
def fetch_subject_and_flags(imap: imaplib.IMAP4, num: bytes) -> tuple[str, str]:
    ...
    _require(typ == "OK", f"FETCH failed: {data}")
    flags = ""
    subject = ""
    for part in data:
...
def check_folder(imap: imaplib.IMAP4, mailbox: str, expected: dict[str, bool]) -> None:
    ...
    _require(typ == "OK", f"cannot SELECT {mailbox}")
    count = int(data[0])
    _require(
        count == len(expected),
    ...
    _require(typ == "OK", f"{mailbox}: SEARCH failed")
    ...
    for subject, want_seen in expected.items():
        _require(subject in seen_subjects, f"{mailbox}: missing subject {subject!r}")
        _require(
            seen_subjects[subject] == want_seen,
...
def main(host: str, port: int, user: str, password: str) -> None:
    ...
    # Exception: the fixture container serves Stalwart's built-in bootstrap certificate.
