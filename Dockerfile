...
RUN set -euo pipefail; \
  source <(grep -hE '^INFINITO_(PYTHON|DOCKER_CLI|PACKAGE|ZFS)_INSTALL_SCRIPT=' "${INFINITO_SRC_DIR}/default.env"); \
  ...
  /bin/bash "${INFINITO_SRC_DIR}/${INFINITO_ZFS_INSTALL_SCRIPT:?}"
