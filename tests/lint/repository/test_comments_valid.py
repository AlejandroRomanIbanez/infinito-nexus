...
Checks ``.py .sh .yml/.yaml .rb .php .css``. Python is parsed with ``tokenize``
...
def find_invalid_comments(path: Path):
    ...
    ext = path.suffix
    if ext == ".py":
        comments = _py_comments(text)
        if comments is None:
            return []
        comments = [(ln, body, {ln}) for ln, body in comments]
    elif ext in (".yml", ".yaml", ".sh", ".rb"):
    ...
    for block in _blocks(comments, lines):
        ...
        block_def = _next_code_is_def(block[-1][2], lines, comment_lines, ext)
...
_EXTS = (".py", ".sh", ".yml", ".yaml", ".rb", ".php", ".css")
