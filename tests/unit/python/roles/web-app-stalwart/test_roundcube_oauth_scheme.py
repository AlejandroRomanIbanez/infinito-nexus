...
TEMPLATE = (
    PROJECT_ROOT / "roles" / "web-app-stalwart" / "templates" / "roundcube-oauth.inc.php.j2"
)
FORCE_HTTPS = "$_SERVER['HTTPS'] = 'on';"
ONION_BASE = "http://webmail.4dl6yijmuksgh5bfaf3csqoelji7zptyuo6wy5g5prjpnk6vdbzfwnad.onion"
...
class TestRoundcubeOauthScheme(unittest.TestCase):
    ...
    def test_an_onion_issuer_routes_the_back_channel_through_tor(self):
        rendered = _render(ONION_BASE, socks_proxy="socks5h://host.docker.internal:9050")
        self.assertIn(
    ...
    def test_nothing_is_emitted_without_sso(self):
        rendered = _render(CLEARNET_BASE, sso_enabled=False, socks_proxy="socks5h://tor:9050")
