"""Unit tests for plugins/lookup/container_networks.py.

Mocks the Ansible-side machinery and asserts that the plugin glue
resolves variables and chains the underlying ``render_container_networks``
function correctly. Rendering correctness itself is covered by
...
from plugins.lookup.container_networks import LookupModule
...
class TestContainerNetworksLookup(unittest.TestCase):
    ...
    def test_falls_back_to_compose_when_deployment_mode_unset(self):
        vars_ = {"application_id": "web-app-x"}
        lm = _make_lookup(vars_)
    ...
    def test_lookup_closures_pass_arguments_in_expected_order(self):
        ...
        def _exercise(*_, lookup_config, lookup_database, **__):
            lookup_config("web-app-x", "services.ldap.enabled", False)
            lookup_database("web-app-x", "shared")
            return "RENDERED"
