...
TEMPLATE = ROLE / "templates" / "javascript.js.j2"
...
class TestLogoutStatusPanel(unittest.TestCase):
    def _collapsed(self) -> str:
        source = read_text(str(TEMPLATE))
        return to_one_liner(re.sub(r"\{\{.*?\}\}", ORIGIN, source, flags=re.DOTALL))
...
class TestPanelTrustsTheFramedOrigin(unittest.TestCase):
    def test_the_panel_trusts_exactly_the_origin_keycloak_is_told_to_frame(self) -> None:
        declaration = next(
            (
                line
        ...
        self.assertTrue(declaration, f"KEYCLOAK_FRONTCHANNEL_LOGOUT_URL missing from {VARS}")
        self.assertIn(
