...
flowchart TB
    ...
    subgraph lab["🌐 Lab bridge 192.168.244.0/24 (MTU 1400)"]
        ...
        nBkp["💽 backup-host (non-swarm)<br/>remote-2-local pull, LUKS device"]:::bkpnode
    end
...
The drill proves the full `svc-bkp-*` chain forward (steps 1-5: real
systemd units + pull + device scripts) and every `recover.py` back
(steps 6-8: device -> local root -> NFS export -> docker volume onto the
live instance), with a marker file that must survive the whole loop. It
reuses the round-1 stack instead of spinning a dedicated cluster, and
skips cleanly when the app declares no NFS-flagged volume. The backup
host is started by `routine/01_bootstrap.sh` (drill profile); the pull trust
(backup keypair) is generated by `utils/tests/swarm/write_extras.py`.
