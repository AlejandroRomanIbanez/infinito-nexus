...
timeout 180 bash "$(dirname "$0")/../unmount_nfs_mounts.sh" "${MGR}" "${WRK1}" "${WRK2}" "${NFS_SERVER}" || true
...
for _node in "${MGR}" "${WRK1}" "${WRK2}" "${NFS_SERVER}" "${BACKUP_NODE}"; do
	timeout 60 docker kill "${_node}" 2>/dev/null
	timeout 30 docker network disconnect -f "${SWARM_LAB_NETWORK}" "${_node}" 2>/dev/null
	timeout 60 docker rm -f "${_node}" 2>/dev/null
	if timeout 30 docker inspect "${_node}" >/dev/null 2>&1; then
