...
#
# A random pick draws only from what every distribution in the entry can
# deliver; a stated one is honoured even where it is unsupported, and the
# applying step then fails. The choice goes to the step summary, since
...
STATED="${1:-}"
LABEL="${2:?usage: resolve.sh STATED LABEL DISTROS}"
DISTROS="${3:-}"
...
candidates() {
	local pool="ext4 btrfs zfs" distro kept fs
...
if [ -n "${STATED}" ]; then
	...
	ORIGIN="random out of '${POOL}' for '${DISTROS}'; re-state it to reproduce this run"
	REQUIRED=false
fi
