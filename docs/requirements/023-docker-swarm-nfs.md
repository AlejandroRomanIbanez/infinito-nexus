...
- [ ] When a target host's `group_names` contains `svc-docker-swarm`, the deploy resolves `deployment_mode = swarm` for that host and renders a Swarm stack file. Otherwise `deployment_mode` stays `compose` (the default).
- [ ] The trigger MUST be inferrable from `group_names` alone, following the dynamic-flag pattern documented in [test_dynamic_flags.py](../../tests/integration/roles/meta/services/test_dynamic_flags.py). No per-role explicit `deployment_mode` override is required to enable swarm.
- [ ] Per-role explicit `deployment_mode: compose` MUST remain possible as an opt-out for a single role on an otherwise-swarm host.
...
- [ ] Among the hosts in `svc-docker-swarm`, the cluster manager is identified by membership in the additional group `svc-docker-swarm-manager`. Exactly one host MUST be a member of `svc-docker-swarm-manager` in v1; all other `svc-docker-swarm` members join as workers. A deploy with zero or more-than-one manager MUST fail at inventory-validation time, not at runtime.

### Role: `svc-docker-swarm`

- [ ] A new role at `roles/svc-docker-swarm/` exists and follows the role-meta layout in [layout.md](../contributing/design/role/services/layout.md) (including `meta/services.yml` with a `lifecycle` key, `meta/schema.yml`, and `tasks/main.yml`).
- [ ] On first application against the host in `svc-docker-swarm-manager`, the role initialises the cluster (`docker swarm init`) with the configured `advertise_addr`, persists the resulting worker and manager join tokens to a documented secret store, and is idempotent on re-runs.
- [ ] On application against any host in `svc-docker-swarm` that is NOT in `svc-docker-swarm-manager`, the role joins the cluster as a worker using the persisted worker token. Joining is idempotent.
- [ ] v1 scope is single-manager. Multi-manager Raft-quorum HA (3+ managers) is explicitly out of scope for this requirement and tracked under [Future Extensions](#future-extensions).
- [ ] Node labels declared for a host in inventory are applied to the joining node (`docker node update --label-add`).
- [ ] The role exposes stack-management primitives (`deploy`, `update`, `remove`) consumable by the deploy pipeline of any role rendering a swarm stack.
- [ ] The role renders a valid `docker-stack.yml` for any role whose target host is in `svc-docker-swarm`, derived from the same per-role inputs as today's `docker-compose.yml`.
- [ ] `deploy.replicas` is taken from a per-role variable with default `1`.
- [ ] `deploy.update_config` is taken from a per-role variable with documented defaults `parallelism: 1`, `delay: 10s`.
- [ ] `deploy.placement.constraints` is generated from inventory node labels when declared on the role.
- [ ] The stack is rendered with **one `docker stack` per role** (consistent with today's one-compose-project-per-role model). Granular `docker stack deploy / update / rm` operations per role MUST be possible. A single global stack across all Infinito services is explicitly rejected.

### Reverse proxy under Swarm: `svc-prx-openresty` stays the edge

- [ ] The existing [svc-prx-openresty](../../roles/svc-prx-openresty/) remains the reverse proxy under BOTH `deployment_mode: compose` AND `deployment_mode: swarm`. No new reverse-proxy role is introduced by this requirement. This preserves the full `sys-front-inj-*` frontend-injection pipeline ([roles/sys-front-inj-all/templates/body_filter.lua.j2](../../roles/sys-front-inj-all/templates/body_filter.lua.j2)) — CSP-meta stripping, head/body snippet injection for Matomo / CSS / JavaScript / Dashboard / Logout — which is openresty-Lua-specific and has no equivalent in Traefik without a non-trivial plugin port.
- [ ] Under `deployment_mode: swarm`, openresty MUST be rendered as a Swarm service pinned to the manager node via `deploy.placement.constraints: node.role == manager`. The edge is single-instance in v1, matching the single-manager scope from Q3. Backend services still benefit from swarm rescheduling; only the ingress edge is pinned.
- [ ] Upstream resolution under Swarm MUST go through the overlay network. openresty's `proxy_pass` targets MUST resolve via Docker Swarm's `tasks.<service>` DNS form (returning ALL healthy task IPs of the service), NOT via the service-name VIP and NOT via fixed container IPs. The `tasks.<service>` form is mandatory because it preserves parity with the existing Compose-mode per-app `upstream` block — openresty performs the load balancing itself in both modes, and the L7-aware path (HTTP healthchecks via `proxy_next_upstream`, future weighting, future sticky sessions) stays under openresty's control. Swarm-native L4 IPVS via service VIP is explicitly rejected for this reason. The injection pipeline (header_filter, body_filter) MUST function unchanged across reschedules of any upstream service.
- [ ] openresty MUST act as the L7 load balancer for every multi-replica backend service. For each web-app service the rendered openresty config MUST contain an `upstream <service> { server tasks.<service>:<port>; }` block, and `proxy_pass` MUST target that upstream block — never the service DNS name inline. v1 uses default round-robin distribution; weighted / sticky / least-conn strategies are out of scope and tracked under [Future Extensions](#future-extensions). The same `upstream` block shape MUST work under both `deployment_mode: compose` (one server entry) and `deployment_mode: swarm` (N task IPs returned by `tasks.<service>`).
- [ ] TLS termination, certificate handling (Let's Encrypt + project CA stack via `sys-front-tls-*`), and all `sys-front-inj-*` snippets MUST work byte-for-byte identically under Swarm and Compose. A diff of the rendered HTML response between Compose and Swarm modes for the pilot role MUST show only expected upstream-dependent variance (URLs, timestamps), never missing or duplicated injection snippets.

### Role: `svc-storage-nfs-server`

- [ ] A new role at `roles/svc-storage-nfs-server/` exists and follows the role-meta layout.
- [ ] When applied to the NFS host, exports are configured under a documented base path (default `/srv/nfs`) with `no_subtree_check` and `sync` semantics.
- [ ] Export ACLs restrict access to the swarm node IPs declared in inventory; an unrelated host MUST NOT be able to mount the export.
- [ ] The role is independently deployable (no implicit pull-in by other roles).

### Role: `svc-storage-nfs-client`

- [ ] A new role at `roles/svc-storage-nfs-client/` exists and follows the role-meta layout.
- [ ] On every host in `svc-docker-swarm`, the role installs the NFS client packages for the target distribution.
- [ ] On every host in `svc-docker-swarm`, the role validates that each configured NFS export from `svc-storage-nfs-server` is mountable, failing the deploy with a precise error message when it is not.
...
- [ ] When a service runs with `deployment_mode: swarm` AND `storage.backend: nfs` AND a specific volume has `nfs: true`, that volume MUST be rendered with the NFS driver block:
...
- [ ] When a service runs with `deployment_mode: swarm` AND has at least one volume NOT opted into NFS (either `nfs: false`, or `storage.backend != nfs`), the deploy MUST emit a clearly-identified warning naming the service and the local-volume names, and MUST pin the service to a single node via a `deploy.placement.constraints` rule on `node.id == <pinned>`. The deploy MUST NOT silently lose data when the service is rescheduled.
- [ ] When a service runs with `deployment_mode: swarm` AND ALL its volumes are opted into NFS AND `storage.backend: nfs`, no placement constraint is added; the service is free to be scheduled on any swarm node.
- [ ] When a service runs with `deployment_mode: compose`, the volume block remains identical to today (local docker volumes, no `driver_opts`); the per-volume `nfs` flag is ignored under compose.

### Docker network integration

- [ ] When a service runs with `deployment_mode: swarm`, every per-role network declared today as `driver: bridge` MUST be rendered as `driver: overlay` with `attachable: true`. Cross-node service-to-service traffic depends on this — `bridge` networks are node-local and would break the swarm mesh.
- [ ] Overlay-network encryption is ON by default for swarm-rendered networks (`driver_opts: { encrypted: "true" }`). It MAY be disabled via a documented `swarm.network.encryption: false` group_var for perf-sensitive deployments. The default ON choice prioritises confidentiality of east-west traffic over throughput.
- [ ] When a service runs with `deployment_mode: compose`, network blocks remain identical to today (`bridge`, no encryption attribute).

### Configuration surface

- [ ] A `storage` block is added to `group_vars` with the shape:
...
- [ ] A `swarm` block is added to `group_vars` with the shape:
...
- [ ] The Ansible inventory MUST express swarm topology purely through group membership. Example:
...
- [ ] A cluster-health check verifies `docker node ls` on the manager reports every expected node as `Ready/Active`.
- [ ] A service-health check verifies every deployed stack reports `replicas: N/N` for the configured replica count.
- [ ] A volume-health check verifies that every NFS-backed mount declared in inventory is reachable from every swarm node.
- [ ] All three checks integrate with the existing `sys-ctl-hlth-*` family (see [roles/sys-ctl-hlth-volumes](../../roles/sys-ctl-hlth-volumes/), [roles/sys-ctl-hlth-container](../../roles/sys-ctl-hlth-container/)) so that operators consume swarm/nfs health the same way as today's health checks.

### Pilot role: `web-app-mediawiki`

- [ ] [web-app-mediawiki](../../roles/web-app-mediawiki/) is the designated pilot role for the Swarm+NFS migration and is documented as such in its `README.md`.
- [ ] Adding the pilot host to the `svc-docker-swarm` group MUST cause MediaWiki to deploy as a Swarm stack. The pilot's volume layout MUST be:
...
- [ ] Rescheduling the MediaWiki **application** service to another swarm node (drain the current node, then `docker service update --force`) MUST preserve all wiki content end to end:
...
- [ ] Every fixture (sample page, sample image, admin credentials) is created inside the workflow run — no external state, no flaky network dependency.
- [ ] The workflow MUST report `failure` (not `skipped`, not `cancelled`) if any of the assertions above fail.
- [ ] The agent MUST iterate on this workflow per [Workflow Loop](../agents/action/iteration/workflow.md) until it passes end to end on the default branch's runner. The requirement is NOT closed while the workflow is red.

### Tests & Documentation

- [ ] `make test` passes with all new roles, schema entries, and lint additions in place.
- [ ] Each new role ships a `README.md` documenting deployment, the swarm/nfs interaction, the configuration surface, and the failure modes (e.g. swarm-without-nfs node pinning).
- [ ] A docs page under [docs/contributing/design/](../contributing/design/) documents the deployment-mode abstraction, the group-membership trigger, and the planned trajectory toward Kubernetes:

  ```
  compose → swarm → kubernetes
  ```

- [ ] The existing backup documentation gains an NFS section covering: snapshot semantics on the NFS server, restore procedure, and interaction with the existing `svc-bkp-*` roles.
